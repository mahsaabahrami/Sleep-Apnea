...
## SLEEP APNEA DETECTION: COMPREHENSIVE ANALYSIS OF MACHINE LEARNING AND DEEP LEARNING METHODS
                                    ## WRITTEN BY: M.BAHRAMI
                                        ## DATE: 12-6-2021
                                ## MODEL: HYBRID MODEL OF AlexNet-BiLSTM
##-----------------------------------------------------------------------------
#IMPORT LIBRARIES    
import pickle
import numpy as np
...
from keras.layers import Dense,Flatten,MaxPooling2D,Conv2D,Permute,Reshape,GRU,BatchNormalization,Bidirectional
...
if __name__ == "__main__":
    ...
    for train, test in kfold.split(X, Y.argmax(1)):
     ...
     model.save(os.path.join("model.AlexNet_BiLSTM.h5"))
